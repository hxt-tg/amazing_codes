from projection import ProjectionSimplexS4
...
simplex_point = s4.project(np.diag([1.2] * 4))
simplex_point_name = ['X', 'Y', 'Z', 'W']

simplex_frame = [
    np.vstack((p1, p2)) for p1, p2 in
    combinations([
        s4.project(np.array([[1., 0., 0., 0.]])),
        s4.project(np.array([[0., 1., 0., 0.]])),
        s4.project(np.array([[0., 0., 1., 0.]])),
        s4.project(np.array([[0., 0., 0., 1.]])),
    ], 2)
]


def plot_data_on_simplex_s4(data, /, style='curve', interval=2, color='b', size=1):
...
if __name__ == '__main__':
    ...
    plot_data_on_simplex_s4(data_4d)
